...
resource "aws_route53_record" "dkim" {
  ...
  name    = "${aws_ses_domain_dkim.client.dkim_tokens[count.index]}._domainkey.${var.domain_name}"
  type    = "CNAME"
  ttl     = 300

  records = ["${aws_ses_domain_dkim.client.dkim_tokens[count.index]}.dkim.amazonses.com"]
}
